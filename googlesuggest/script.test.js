...
describe("Google Suggest Explorer", () => {
  ...
  beforeEach(async () => {
    ...
    document.getElementById("openaiBaseUrl").value =
      localStorageMock.getItem("openai_base_url") || "https://api.openai.com/v1";
  ...
  describe("Google Suggest Fetching", () => {
    it("fetches and displays suggestions when button is clicked", async () => {
      ...
      for (let i = 0; i < 8; i++) {
        // Mock remaining 8 countries as success to avoid timeouts
      ...
      const usCard = Array.from(countryCards).find((card) =>
        card.querySelector(".card-header.country-name").textContent.includes("United States"),
      );
      ...
      const gbCard = Array.from(countryCards).find((card) =>
        card.querySelector(".card-header.country-name").textContent.includes("United Kingdom"),
      );
      ...
      expect(localStorageMock.setItem).toHaveBeenCalledWith(
        expect.stringContaining("googleSuggest_v1.1_test query"),
        expect.any(String),
      );
  ...
  describe("LLM Explanation", () => {
    it("calls asyncLLM and displays explanation", async () => {
      ...
      for (let i = 0; i < 9; i++) {
        simulateJsonpResponse(["llm test", ["s" + (i + 2)]]);
        await window.happyDOM.whenAsyncComplete();
      }
      await window.happyDOM.whenAsyncComplete();
      ...
      expect(localStorageMock.setItem).toHaveBeenCalledWith(
        expect.stringContaining("llmExplanation_v1.1_llm test"),
        expect.stringContaining('"Humorous explanation."'),
      );
      ...
      expect(localStorageMock.setItem).toHaveBeenCalledWith(
        expect.stringContaining("llmExplanation_v1.1_llm test"),
        expect.stringContaining('"Proxy explanation."'),
      );
    });
  });
  ...
  describe("Caching", () => {
    ...
    it("loads LLM explanation from cache", async () => {
      ...
      const suggestionsHash = JSON.stringify(suggestions)
        .split("")
        .reduce((a, b) => {
  ...
  describe("Search History", () => {
    it("adds to search history and displays it", async () => {
      ...
      for (let i = 0; i < 9; i++) {
        simulateJsonpResponse(["history query", ["s" + (i + 2)]]);
        await window.happyDOM.whenAsyncComplete();
      }
      await window.happyDOM.whenAsyncComplete();

      expect(localStorageMock.setItem).toHaveBeenCalledWith(
        expect.stringContaining("searchHistory_v1.1"),
        expect.stringContaining("history query"),
      );
      ...
      initHistory.forEach((query) => {
        ...
        deleteButton.addEventListener("click", (event) => {
          ...
          currentHistory.forEach((q_new) => {
            const new_el_btn = document.createElement("button");
            new_el_btn.textContent = q_new;
            new_el_btn.className = "btn btn-sm btn-outline-info history-term-button";
            const new_del_btn = document.createElement("button");
            new_del_btn.innerHTML = '<i class="bi bi-x-lg"></i>';
            new_del_btn.className = "btn btn-sm btn-outline-danger delete-history-item";
            const new_grp = document.createElement("div");
            new_grp.className = "history-item-group";
            new_grp.appendChild(new_el_btn);
            new_grp.appendChild(new_del_btn);
  ...
  describe("LLM Settings Persistence", () => {
    ...
    it("loads LLM settings from localStorage on init", async () => {
      ...
      expect(document.getElementById("llmModel").value).toBe(
        localStorageMock.getItem("llm_model") || "openai/gpt-4.1-mini",
      );
      expect(document.getElementById("openaiBaseUrl").value).toBe(
        localStorageMock.getItem("openai_base_url") || "https://custom.openai.com/v1",
      ); // This specific mock value is fine
      expect(document.getElementById("openaiApiKey").value).toBe(
        localStorageMock.getItem("openai_api_key") || "ls-api-key",
      ); // This specific mock value is fine
    });
  });
});
