...
const loadBookmarklet = async () => {
  ...
  showError("");
  try {
    let scriptPath = "githubissuescraper.js";
    try {
    ...
    const bookmarkletCode = `${code};githubissuescraper.scrape();`;
    bookmarkletHref = `javascript:${encodeURIComponent(bookmarkletCode)}`;
    if (bookmarkletButton) bookmarkletButton.href = bookmarkletHref;
    updateStatus("Drag the button to your bookmarks bar, then click it on a GitHub issue page.");
  } catch (error) {
