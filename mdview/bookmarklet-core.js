...
export const extractMarkdownSource = (doc = document, win = window) => {
...
export const buildPopupHtml = ({ markdown, title, sourceUrl, isSelection }) => {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .eyebrow {
      margin: 0 0 8px;
      font: 600 0.82rem/1.4 "IBM Plex Sans", "Segoe UI", sans-serif;
    ...
    .meta {
      margin: 12px 0 0;
      color: var(--muted);
      font: 500 0.98rem/1.6 "IBM Plex Sans", "Segoe UI", sans-serif;
    }
...
export const openPopup = (payload, win = window) => {
...
export const showPopup = (doc = document, win = window) => openPopup(extractMarkdownSource(doc, win), win);

export const buildBookmarkletHref = (code) => `javascript:${encodeURIComponent(`${code};mdview.showPopup();`)}`;
