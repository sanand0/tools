...
describe("findsongs playlist", () => {
  let window, document, preferencesInput, generateBtn, refineBtn, playlistContainer, copyBtn;
  ...
  it("generates playlist items from LLM output", async () => {
    ...
    window.__setLLM(async function* (url, init) {
      ...
      yield { content: '["Song Alpha — Artist", "Song Beta — Artist"]' };
    });
  ...
  it("retains rated songs after refining the playlist", async () => {
    const sequence = [
      '["Song A — Artist", "Song B — Artist", "Song C — Artist"]',
      '["Song D — Artist", "Song E — Artist", "Song F — Artist"]',
    ];
    let callIndex = 0;
    const bodies = [];
  ...
  it("copies the playlist to clipboard", async () => {
    window.__setLLM(async function* () {
      yield { content: '["Song One", "Song Two"]' };
    });
